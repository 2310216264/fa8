...
$c=returncount("yjcode_order where selluserid=".$rowuser[id]." and admin=1 and (ddzt='wait' or ddzt='db' or ddzt='suc') and sj>='".$sj1."' and sj<='".$sj."'");
$f=sprintf("%.2f",returnsum("allmoney3","yjcode_order where selluserid=".$rowuser[id]." and admin=1 and (ddzt='wait' or ddzt='db' or ddzt='suc') and sj>='".$sj1."' and sj<='".$sj."'"));
...
<span class="s3">
<? if(1==$rowuser[ifmot]){?>
<img src="<?=weburl?>shop/img/sj1.gif" title="手机已通过认证" /><? }else{?><img src="<?=weburl?>shop/img/sj0.gif" title="手机未认证" />
<? }?>
...
<span class="s4">
	[ 
		描述：<span <? if($mspf<4.8){?>class="green"<?}else{?>class="red"<?}?> ><?=$mspf?></span> | 
		发货：<span <? if($fhpf<4.8){?>class="green"<?}else{?>class="red"<?}?> ><?=$fhpf?></span> | 
		售后：<span <? if($shpf<4.8){?>class="green"<?}else{?>class="red"<?}?> ><?=$shpf?></span> 
	]
</span>
<!--拉框B-->
...
<?
while1("*","yjcode_ad where adbh='ADSHOP01' and zt=0 order by xh asc limit 1");$row1=mysqli_fetch_array($res1);
...
<div class="d1"><a href="<?=weburl?>shop/prolist_i<?=$uid?>v.html" class="a1">所有商品</a></div>
<div class="d1"><a href="<?=weburl?>shop/pjlist_i<?=$uid?>v.html" class="a1">商品点评</a></div>
<div class="d1"><a href="<?=weburl?>shop/serlist_i<?=$uid?>v.html" class="a1">我的服务</a></div>
<div class="d1"><a href="<?=weburl?>shop/aboutview<?=$uid?>.html" class="a1">关于我们</a></div>
<? }else{?>
<? 
while1("*","yjcode_shopmenu where zt=0 and admin=1 and userid=".$uid);while($row1=mysqli_fetch_array($res1)){
...
<? 
while2("*","yjcode_shopmenu where zt=0 and admin=2 and tit1='".$row1[tit1]."' and userid=".$uid);while($row2=mysqli_fetch_array($res2)){

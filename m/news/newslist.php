...
$getstr=$_GET[str];
$ty1id=intval(returnsx("j"));
$ty2id=intval(returnsx("k"));
$ses=" where zt=0";
...
if(returnsx("s")!=-1){$skey=returnsx("s");$ses=$ses." and (tit like '%".$skey."%' or wkey like '%".$skey."%')";}
?>
<html>
<head>
...
<div class="sertj box" id="sertj2" style="display:none;">
 ...
 <? while1("*","yjcode_newstype where admin=2 and name1='".$ty1name."' order by xh asc");while($row1=mysqli_fetch_array($res1)){?>
...
<div class="listtop box">
 <div class="d1" onClick="gourl('../search/main.php?admin=5')"><span class="s1"><img src="../img/ser.png" /></span><span class="s2"><?=returnjgdw($skey,"","请输入搜索关键词！")?></span></div>
</div>

<div class="yjcode">
<? 
$i=1;pagef($ses,10,"yjcode_news","order by lastsj desc");while($row=mysqli_fetch_array($res)){
...
<div class="ilist box" onClick="gourl('txtlist_i<?=$row[id]?>v.html');">
 <div class="d1"><img src="<?=$tpv?>" onerror="this.src='../../img/none180x180.gif'" /></div>
 <div class="d2">

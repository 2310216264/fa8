...
$id=intval($_GET[id]);
while0("*","yjcode_gg where id=".$id);if(!$row=mysqli_fetch_array($res)){php_toheader("gglist.html");}
checkdjl("c2",$row[id],"yjcode_gg");

while1("*","yjcode_gg where sj>='".$row[sj]."' and id<>".$row[id]." and zt<>99 order by sj asc");
if($row1=mysqli_fetch_array($res1)){$pre="<a href='ggview".$row1[id].".html' class='a1'>上一篇</a>";}
while1("*","yjcode_gg where sj<='".$row[sj]."' and id<>".$row[id]." and zt<>99 order by sj desc");
if($row1=mysqli_fetch_array($res1)){$nex="<a href='ggview".$row1[id].".html' class='a2'>下一篇</a>";}
?>
<html>
<head>

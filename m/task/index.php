...
$ty1id=returnsx("j");if($ty1id!=-1){
 $ses=$ses." and type1id=".$ty1id;
 $ty1name=returntasktype(1,$ty1id);
 $sqlty1="select * from yjcode_tasktype where admin=1 and id=".$ty1id;mysqli_set_charset($conn,"utf8");$resty1=mysqli_query($conn,$sqlty1);$rowty1=mysqli_fetch_array($resty1);
...
$ty2id=returnsx("k");if($ty2id!=-1){$ses=$ses." and type2id=".$ty2id;$ty2name=returntasktype(2,$ty2id);$lastty=$ty2name;$tit=$tit."/".$ty2name;}
if(returnsx("s")!=-1){
...
<div class="sertj box" id="sertj1" style="display:none;">
 ...
 <a href="<?=rentser('j',$row1[id],'k','');?>" <? if(check_in("_j".$row1[id]."v",$getstr)){?> class="nx"<? }?>><?=$row1[name1]?></a>
 <? }?>
 </div>
 ...
 <? while2("*","yjcode_tasktype where admin=2 and name1='".$ty1name."' order by xh asc");while($row2=mysqli_fetch_array($res2)){?>

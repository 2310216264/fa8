...
//$fp= fopen("1.txt","w");
//fwrite($fp,$objectxml["trade_state"]);
//fclose($fp);

if($objectxml["trade_state"]=="SUCCESS"){
 //自己逻辑代码B
 $sql="select * from yjcode_dingdang where ddbh='".$out_trade_no."' and ifok=0";mysqli_set_charset($conn,"utf8");$res=mysqli_query($conn,$sql);
 if($row=mysqli_fetch_array($res)){
  if(1==$row[ifok]){echo "success";exit;}
  $sj=date("Y-m-d H:i:s");
  $uip=$_SERVER["REMOTE_ADDR"];
  if(round($total_fee/100,2)!=round($row["money1"],2)){echo "success";exit;}

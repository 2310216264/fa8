...
<div class="dmain flex">
 <div class="swiper-container" id="swiper1">
  <div class="swiper-wrapper">
   <div class="swiper-slide"><a href="serverorder.php" class="<? if($_GET[ddzt]==""){?> a1<? }?>">所有订单</a></div>
   <? for($i=1;$i<=13;$i++){?>
   <div class="swiper-slide"><a href="serverorder.php?ddzt=<?=$i?>" class="<? if($_GET[ddzt]==$i){?> a1<? }?>"><?=strip_tags(returnserverorderzt($i))?></a></div>
   <? }?>
  </div>
 </div>
</div>
...
<? include("topuser.php");?>
 ...
 <!--列表开始-->
 <?
 pagef($ses,20,"yjcode_serverorder","order by sj desc");while($row=mysqli_fetch_array($res)){
 $tp=returntp("bh='".$row[serverbh]."' order by xh desc","-1");
 $au="serverorderview.php?orderbh=".$row[orderbh];
 $cz="";
 if($row[ddzt]==1){
 $cz="<a href='serverclose.php?orderbh=".$row[orderbh]."'>取消订单</a>";
 
 }elseif($row[ddzt]==2){ //商家已同意，等待买家付款
 $cz="<a href='orderpay.php?orderbh=".$row[orderbh]."' class='a1'>支付费用</a>";
 ...
 $cz="<a href='serverys.php?orderbh=".$row[orderbh]."' class='a1'>前往验收</a>";
 $cz=$cz."<a href='servertk.php?orderbh=".$row[orderbh]."'>申请退款</a>";
 
 }elseif($row[ddzt]==6){ //交易成功
 
 }elseif($row[ddzt]==7){ //退款处理中
 $cz="<a href='serverqxtk.php?orderbh=".$row[orderbh]."' class='a1'>取消退款</a>";
 $cz=$cz."<a href='serverorderjf1.php?orderbh=".$row[orderbh]."'>沟通记录</a>";

 }elseif($row[ddzt]==8){ //退款成功
 $cz="<a href='serverorderjf1.php?orderbh=".$row[orderbh]."'>沟通记录</a>";

 }elseif($row[ddzt]==9){ //退款不同意
 $cz="<a href='serverjf.php?orderbh=".$row[orderbh]."' class='a1'>平台介入</a>";
 $cz=$cz."<a href='serverorderjf1.php?orderbh=".$row[orderbh]."'>沟通记录</a>";
 ...
 $cz="<a href='serverorderjf1.php?orderbh=".$row[orderbh]."'>沟通记录</a>";
  ...
  $cz="<a href='serverorderpj.php?orderbh=".$row[orderbh]."' class='a1'>评价</a>";
  }else{
  $cz="<a href='serverorderpj.php?orderbh=".$row[orderbh]."'>已评价</a>";
  }
 $cz=$cz."<a href='serverorderjf1.php?orderbh=".$row[orderbh]."'>沟通记录</a>";
 ...
 <div class="serverorderlist1 box" onClick="gourl('<?=$au?>')">

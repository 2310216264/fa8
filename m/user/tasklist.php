...
<div class="listcontrol box">
 ...
 pagef($ses,20,"yjcode_task","order by sj desc");while($row=mysqli_fetch_array($res)){
 while1("*","yjcode_taskhf where bh='".$row[bh]."' order by money1 desc");if($row1=mysqli_fetch_array($res1)){$moneyg=$row1[money1];}$moneyg=returnjgdw($moneyg,"",0);
 while1("*","yjcode_taskhf where bh='".$row[bh]."' order by money1 asc");if($row1=mysqli_fetch_array($res1)){$moneyd=$row1[money1];}$moneyd=returnjgdw($moneyd,"",0);

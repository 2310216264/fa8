...
$sqltask="select * from yjcode_task where bh='".$bh."' and taskty=0 and useridhf=".$userid."";mysqli_set_charset($conn,"utf8");$restask=mysqli_query($conn,$sqltask);
if(!$rowtask=mysqli_fetch_array($restask)){php_toheader("taskhflist.php");}

$sqltaskhf="select * from yjcode_taskhf where bh='".$bh."' and taskty=0 and useridhf=".$rowtask[useridhf]." and ifxz=1";mysqli_set_charset($conn,"utf8");$restaskhf=mysqli_query($conn,$sqltaskhf);
if(!$rowtaskhf=mysqli_fetch_array($restaskhf)){php_toheader("taskhflist.php");}
...
<div class="gtlistv box">
 ...
 <? 
 while1("*","yjcode_tasklog where bh='".$bh."' order by sj asc");while($row1=mysqli_fetch_array($res1)){
 $txt=$row1[txt];

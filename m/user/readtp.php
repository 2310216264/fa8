...
if(!$rowuser=mysqli_fetch_array($resuser)){exit;}
$userid=$rowuser[id];
?>
...
<? 
while1("*","yjcode_tp where bh='".$_GET[bh]."' order by xh asc");while($row1=mysqli_fetch_array($res1)){
if(empty($row1[upty])){
 $tp1="../../".$row1[tp];
...
<? while1("*","yjcode_tp where bh='".$_GET[bh]."' order by xh asc");while($row1=mysqli_fetch_array($res1)){$tp="../../".str_replace(".","-1.",$row1[tp]);?>
<div class="d1">

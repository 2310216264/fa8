...
if($admin==1){ //店铺LOGO
 ...
 $a="../../upload/".$userid."/".strgb2312($rowuser[qyzch],0,15)."-zz.jpg";
 if(is_file($a)){$tp=$a."?t=".time();}
...
if($_GET[action]=="update"){  //上传
 ...
 createDir("../../".$targetFolder);
  ...
  $targetFile = "../../upload/".$userid."/".strgb2312($rowuser[qyzch],0,15)."-zz.jpg";
  move_uploaded_file($_FILES['inp1']['tmp_name'],$targetFile);
  $cm=new CreatMiniature();
  $bw=800;$bg=0;
  $imgsrc=$targetFile;
  list($width, $height) = getimagesize(weburl."upload/".$userid."/".strgb2312($rowuser[qyzch],0,15)."-zz.jpg");
  $bgv=intval($height/($width/$bw));
  $cm->SetVar($imgsrc,"file");if($width>$bw){$cm->BackFill($imgsrc,$bw,$bgv);}

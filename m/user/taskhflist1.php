...
if($_GET[control]=="ok"){ //完成
 $bh=$_GET[bh];
 while0("*","yjcode_taskhf where bh='".$bh."' and taskty=1 and id=".intval($_GET[id])." and useridhf=".$userid);if(!$row=mysqli_fetch_array($res)){php_toheader("taskhflist1.php");}
 $sj=date("Y-m-d H:i:s");
 $txt="已经完成任务，发起验收申请";
 intotable("yjcode_tasklog","bh,userid,useridhf,admin,txt,sj,fj","'".$bh."',".$row[userid].",".$userid.",2,'".$txt."','".$sj."',''");
 updatetable("yjcode_taskhf","zt=1 where id=".intval($_GET[id])." and zt=0 and useridhf=".$userid);
 php_toheader("taskhflist1.php");
 
}elseif($_GET[control]=="pt"){ //申请平台介入
 while0("*","yjcode_taskhf where id=".intval($_GET[id])." and taskty=1 and zt=3 and useridhf=".$userid);if(!$row=mysqli_fetch_array($res)){php_toheader("taskhflist1.php");}
...
<div class="bfbtop1 box">
 ...
 pagef($ses,30,"yjcode_taskhf","order by sj desc");while($row=mysqli_fetch_array($res)){
 while1("*","yjcode_task where bh='".$row[bh]."'");$row1=mysqli_fetch_array($res1);

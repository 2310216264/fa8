...
<div class="infmain box">
 <div class="d1">
  ...
  <? $a=returncount("yjcode_order where userid=".$rowuser[id]." and ddzt='wait' and admin=1");?>
  <div onClick="gourl('order.php?ddzt=wait')"><img src="img/order2.png" /><? if($a>0){?><span><?=$a?></span><? }?><br>等待发货</div>
  <? $a=returncount("yjcode_order where userid=".$rowuser[id]." and ddzt='db' and admin=1");?>
  <div onClick="gourl('order.php?ddzt=db')"><img src="img/order3.png" /><? if($a>0){?><span><?=$a?></span><? }?><br>等待收货</div>
  <? $a=returncount("yjcode_order where userid=".$rowuser[id]." and ddzt='back' and admin=1");?>
  <div onClick="gourl('order.php?ddzt=back')"><img src="img/order4.png" /><? if($a>0){?><span><?=$a?></span><? }?><br>退款处理</div>
  <? $a=returncount("yjcode_order where userid=".$rowuser[id]." and ddzt='jf' and admin=1");?>

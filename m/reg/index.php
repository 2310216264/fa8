...
function regtyonc(x){
 if(0==<?=intval($rowcontrol[mrbuy])?>){
 document.getElementById("regty").style.display="none";
 return false;
 }
 for(i=1;i<=2;i++){
  document.getElementById("miancap"+i).className="d1";
  document.getElementById("mianv"+i).style.display="none";
 }
  document.getElementById("miancap"+x).className="d1 d11";
...
<div class="regty box" id="regty">
 <div class="d1 d11" onClick="regtyonc(2)" id="miancap2">常规方式登录</div>
 <div class="d1" onClick="regtyonc(1)" id="miancap1">免登录购买</div>
</div>
...
<div id="mianv1" style="display:none;">
...
if(!empty($wxpay[0]) && $rowcontrol[wxlogin]!="" && $rowcontrol[wxlogin]!=","){
php_toheader("https://open.weixin.qq.com/connect/oauth2/authorize?appid=".$wxpay[0]."&redirect_uri=".urlencode(weburl."m/reg/wxlogin.php")."&response_type=code&scope=snsapi_userinfo&connect_redirect=1#wechat_redirect");//自动微信登录
}}
?>

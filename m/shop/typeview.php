...
<? while1("*","yjcode_protype where admin=1 and userid=".$uid." and zt=0 order by xh asc");while($row1=mysqli_fetch_array($res1)){?>
<div class="type1 box" onClick="gourl('prolist_i<?=$uid?>v_j<?=$row1[id]?>v.html')"><div class="d1"><?=$row1[name1]?></div></div>
 <div class="type2 box">
 <div class="dm">
 <? while2("*","yjcode_protype where admin=2 and userid=".$uid." and zt=0 and name1='".$row1[name1]."' order by xh asc");while($row2=mysqli_fetch_array($res2)){?>

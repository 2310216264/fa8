...
function sqlzhuru($str) { //去除所有html标签
        $str=str_ireplace("'","’",$str);
        $str=str_ireplace("\"","”",$str);
        return strip_tags(gloguolv($str));
}

function sqlzhuru1($str){ //编辑器，保留常规HTML格式，用于编辑器
        $content=gloguolv($str);
        $content=str_ireplace("<script","&lt;script",$content);
        $content=str_ireplace("<iframe","&lt;iframe",$content);
        $content=str_ireplace("<style","&lt;style",$content);
        return $content;
}

function sqlzhuru2($str){ //编辑器，仅保留script，用于统计代码等添加
        $content=gloguolv($str);
        $content=str_ireplace("<iframe","&lt;iframe",$content);
        $content=str_ireplace("<style","&lt;style",$content);
        return $content;
...
function returnsjc(){return strtotime(getsj());}

function returnsj($x){
if(empty($x)){return "";}else{return date('Y-m-d H:i:s',$x);}
}

function returnwan($x){
 if($x>10000){$s=$x/10000;return $s."万";}
 else{return $x;}
}
...
function returndqsj($x){
 if($x=="2099-12-30 00:00:00"){echo "永不到期";}elseif(empty($x)){return "永不到期";}else{return $x;}
}
...
function yjImgToBase64($img_file) {
...
function getuip($type = 0, $adv = false) {
        $type       =  $type ? 1 : 0;
        static $ip  =   NULL;
...
function js_unescape($str){return $str;}

function DateDiff($date1, $date2, $unit = "") {switch($unit){case 's':$dividend = 1;break;case 'i':$dividend = 60;break;case 'h':$dividend = 3600;break;case 'd':$dividend = 86400;break;default:$dividend = 86400;}$time1 = strtotime($date1);$time2 = strtotime($date2);if ($time1 && $time2) return (float)($time1 - $time2) / $dividend;return false;}function read_file_content($FileName) {$fp=fopen($FileName,"r"); $data=""; while(!feof($fp)) {$data.=fread($fp,4096); } fclose($fp); return $data; }
 
 
 function returnsxs($x){
	$nstr=sqlzhuru($_GET[id]);
	if(check_in("_".$x,$nstr)){
		$re1=preg_split("/_".$x."/",$nstr);
		$re3=preg_split("/_/",$re1[1]);
		$re2=preg_split("/v/",$re3[0]);
		$ssr="";
		for($ri=0;$ri<count($re2);$ri++){
			$ssr=$ssr.$re2[$ri];
			if($ri<(count($re2)-2)){
				$ssr=$ssr."v";
...
function check_in($arr, $text){$keys = explode(',',$arr);$result = 0;if($keys){foreach($keys as $key){if(strstr($text,$key)!=''){$result = 1;break;}}}return $result;}function returnjgdw($m,$d,$t="面议"){if(empty($m)){return $t;}else{return $m.$d;}}
 
function returntppd($tp1,$tp2){if(is_file($tp1)){return $tp1;}else{return $tp2;}} //因为引入了OSS，所以这个图片判断函数基本失效了
 
function safeEncoding($string){return $string;}
...
function returntitcss($t,$b,$c){$tit=$t;if(1==$b){$tit="<strong>".$tit."</strong>";}if(!empty($c) && $c!="#333"){$tit="<font color='".$c."'>".$tit."</font>";}return $tit;}function returntitdian($t,$l){$len=strlen($t);if($len>$l){return strgb2312($t,0,$l-2)."...";}else{return $t;}}

function returnztv($zv,$zvsm=""){if(0==$zv){$ztv="<span class='blue'>已通过审核</span>";}elseif(1==$zv){$ztv="<span class='feng'>正在审核</span>";}elseif(2==$zv){$ztv="<span class='red'>审核不通过,".$zvsm."</span>";}return $ztv;}


function htmlget($url){$ch = curl_init();curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, FALSE);curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, FALSE);curl_setopt($ch, CURLOPT_URL, $url);curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);curl_setopt($ch, CURLOPT_REFERER, CHR);curl_setopt($ch, CURLOPT_HEADER,0);$output = curl_exec($ch);curl_close($ch);return $output;}

function systs($a,$b){if($_GET[t]=="suc"){echo "<div class=\"systs\">".$a."[<a href=\"".$b."\">知道了</a>]</div>";}}
...
function strgb2312($a,$b,$c){
return mb_substr($a,$b,$c,"utf-8");
}
function strgb2312s($a,$b,$c){
return mb_substr($a,$b,$c,"utf-8").'...';
}

function dateYMDN($m){$a=preg_split("/\s/",$m);$b=str_replace("-","",$a[0]);$b=str_replace("/","",$b);return $b;}
...
function returnonecon($x){
 ...
 elseif(10==$x){return "服务市场交易流程";}
}

function dateYMD($m){$a=preg_split("/\s/",$m);return $a[0];}

function dateYM($m){$a=dateYMD($m);$b=preg_split("/-/",$a);$yv=$b[0];$mv=$b[1];return $yv."-".$mv;}

function dateMD($m){$a=dateYMD($m);$b=preg_split("/-/",$a);$mv=$b[1];$dv=$b[2];return $mv."/".$dv;}

function dateYMDHM($m){$a=preg_split("/:/",$m);return $a[0].":".$a[1];}function is_date($date){if($date == date('Y-m-d H:i:s',strtotime($date))){return true;}else{return false;}}
...
function returnsfzzt($x){
 if(empty($x)){return "<span class='feng'>等待验证</span>";}
 elseif(1==$x){return "<span class='blue'>通过认证</span>";}
 elseif(2==$x){return "<span class='red'>认证被拒</span>";}
 elseif(3==$x){return "<span class='hui'>未提交认证</span>";}
}

function returnzzrzzt($x){
 if(empty($x)){return "<span class='feng'>等待验证</span>";}
 elseif(1==$x){return "<span class='blue'>通过认证</span>";}
 elseif(2==$x){return "<span class='red'>认证被拒</span>";}
 elseif(3==$x){return "<span class='hui'>未提交认证</span>";}
...
function returnserverorderzt($zv){
if($zv==1){
$ztv="<span class='hui'>商家确认</span>";
}elseif($zv==2){
$ztv="<span class='red'>待付款</span>";
}elseif($zv==3){
$ztv="<span class='hui'>商家关闭订单</span>";
}elseif($zv==4){
$ztv="<span class='feng'>担保中</span>";
}elseif($zv==5){
$ztv="<span class='red'>待买家验收</span>";
}elseif($zv==6){
$ztv="<span class='green'>交易成功</span>";
}elseif($zv==7){
$ztv="<span class='red'>退款处理中</span>";
}elseif($zv==8){
$ztv="<span class='blue'>退款成功</span>";
}elseif($zv==9){ 
$ztv="<span class='red'>不同意退款</span>";
}elseif($zv==10){
$ztv="<span class='feng'>平台介入</span>";
}elseif($zv==11){
$ztv="<span class='green'>商家胜诉</span>";
}elseif($zv==12){
$ztv="<span class='green'>买家胜诉</span>";
}elseif($zv==13){
$ztv="<span class='hui'>买家关闭订单</span>";
}
return $ztv;
}
...
function returnqylx($x,$y){
if(intval($y)!=1){
 return "个人";
}else{
 if(empty($x)){return "个人";}
 elseif($x==1){return "个体户";}
 elseif($x==2){return "企业";}
}
}

function returnqylx1($x){
 if(empty($x)){return "个人";}
 elseif($x==1){return "个体户";}
 elseif($x==2){return "企业";}
}

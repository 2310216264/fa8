...
while0("*","yjcode_news where id=".$id);if(!$row=mysqli_fetch_array($res)){php_toheader(weburl."404.html");exit;}
...
if($row1=mysqli_fetch_array($res1)){$pre="<a href='txtlist_i".$row1[id]."v.html'>".$row1[tit]."</a>";}
else{$pre="已是第一篇内容";}
...
if($row1=mysqli_fetch_array($res1)){$nex="<a href='txtlist_i".$row1[id]."v.html'>".$row1[tit]."</a>";}
else{$nex="已是最后一篇内容";}
...
while1("*",$ses." and zt=1 and userid=".$userid." order by sj desc");while($row1=mysqli_fetch_array($res1)){
...
<? 
while1("*",$ses." and zt=0 order by sj desc limit 10");while($row1=mysqli_fetch_array($res1)){
$usertx="../upload/".$row1[userid]."/user.jpg";
if(!is_file($usertx)){$usertx="../user/img/nonetx.gif";}
?>
<div class="pld">
...
<? while1("*","yjcode_news where zt=0 order by lastsj desc limit 10");while($row1=mysqli_fetch_array($res1)){?>

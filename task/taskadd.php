...
if($_GET[control]=="add"){
 ...
 intotable("yjcode_task","bh,userid,sj,lastsj,zt,tit,txt,type1id,type2id,jgxs,money1,money2,money3,money4,money5,djl,useridhf,rwzq,yxq,yjtx,qqxs,motxs,yjfs,fj,taskty,tasknum,taskcy,jsbao","'".$bh."',".$userid.",'".$sj."','".$sj."',".intval($ztv).",'".sqlzhuru($_POST[t1])."','".sqlzhuru1($_POST[content])."',".intval($ty[0]).",".intval($ty[1]).",".intval($jgxs).",".$money1.",".$money2.",0,0,0,0,0,".intval($zq).",'".$endsj."',".intval($_GET[yjtz]).",".intval($_POST[qqxsinp]).",".intval($_POST[motxsinp]).",".intval($_POST[R4]).",'".$mc."',".intval($rwxs).",".intval($renshu).",0,".$jsbao."");
...
<div class="yjcode">
 ...
 <div class="tleft fontyh">
  ...
  <? while2("*","yjcode_tasktype where admin=2 and name1='".$row1[name1]."' order by xh asc");while($row2=mysqli_fetch_array($res2)){?>

<?
 ...
 if(!empty($_COOKIE['tjuserid'])){$tu=intval($_COOKIE['tjuserid']);}
 $ifmot=returnjgdw($ifmot,"",0);
 if(empty($openid)){$ifqq=0;}else{$ifqq=1;}
 $sqli="insert into yjcode_user(uid,pwd,sj,uip,baomoney,money1,jf,nc,mot,ifmot,email,ifemail,uqq,yxsj,openid,ifqq,djl,pm,zt,openshop,shopzt,zfmm,sellmall,sellmyue,tjuserid,sellbl,sfzrz,wxopenid,unionid,ifmian)values('".$uid."','".sha1($pwd)."','".getsj()."','".getuip()."',0,0,0,'".$nc."','".$mot."',".$ifmot.",'".$email."',0,'".$uqq."','2014-10-15','".$openid."',".$ifqq.",0,0,1,0,0,'".sha1($pwd)."',0,0,".intval($tu).",".$rowcontrol[sellbl].",3,'".$wxopenid."','".$unionid."',".intval($ifmian).")";
 mysqli_set_charset($conn,"utf8");mysqli_query($conn,$sqli);

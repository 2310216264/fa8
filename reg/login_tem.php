...
if(empty($uid) || empty($pwd)){Audit_alert("帐号或密码输入有误，返回重试","./");}
 $sql="select * from yjcode_user where (uid='".$uid."' or (mot='".$uid."' and ifmot=1)) and pwd='".sha1($pwd)."'";mysqli_set_charset($conn,"utf8");$res=mysqli_query($conn,$sql);
 ;if(!$row=mysqli_fetch_array($res)){Audit_alert("帐号密码验证错误，请返回重试","./");}

...
if($_GET[action]=="login" && sqlzhuru($_POST[jvs])=="login"){
 ...
 while0("*","yjcode_yzm where tit='".$mot."' and yzm='".sqlzhuru($_POST[yzm])."' and admin=2");if(!$row=mysqli_fetch_array($res)){Audit_alert("短信验证码输入有误，返回重试","index.php");}
 deletetable("yjcode_yzm where tit='".$mot."'");
 $sj=getsj();
 $uip=getuip();
 while1("*","yjcode_user where mot='".$mot."' and ifmot=1");if($row1=mysqli_fetch_array($res1)){
...
<? while1("*","yjcode_ad where adbh='ADO01' and zt=0 order by xh asc");if($row1=mysqli_fetch_array($res1)){$a="../".returnjgdw($rowcontrol[addir],"","gg")."/".$row1[bh].".".$row1[jpggif];}?>

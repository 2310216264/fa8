...
.tangqq .u1 .l4{width:260px;padding:10px 0 0 60px;}
.tangqq .u1 .l4 img{width:140px;}
</style>
</head>
<body>
<div class="tangqq">
 ...
 <? if(!empty($qq)){?>
 <li class="l2"><a href="tencent://AddContact/?fromId=50&fromSubId=1&subcmd=all&uin=<?=$qq?>" target="_blank" class="a1">加为好友</a><a href="http://wpa.qq.com/msgrd?v=3&uin=<?=$qq?>&site=qq:<?=$qq?>&menu=yes" target="_blank" class="a2">直接对话</a></li>
 <? }?>

 <? if(!empty($wx)){$height=$height+30;?>
 <li class="l1"><strong>微信号码：</strong><?=$wx?></li>
 <? }?>
 
 <? if(is_file($tp)){$height=$height+150;?>
 <li class="l4"><img src="<?=$tp?>" /></li>
 <? }?>
 
 <li class="l3">声明：以上信息是用户提供，平台不确保其真实性，涉及资金交易时，请核实所要购买的商品与以上信息是否有关联（谨防号码无效、冒用等情况）</li>
 </ul>
</div>
<script language="javascript">
var index = top.layer.getFrameIndex(window.name);
top.layer.style(index,{height:"<?=$height?>px"});
</script>
</body>
</html>
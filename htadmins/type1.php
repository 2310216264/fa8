...
intotable("yjcode_type","admin,type1,xh,sj,col,iftj,tjmoney,sellbl,seotit,seokey,seodes,propx","1,'".sqlzhuru($_POST[t1])."',".sqlzhuru($_POST[t2]).",'".$sj."','".sqlzhuru($_POST[tcol])."',".sqlzhuru($_POST[R1]).",".abs($_POST[ttjmoney]).",".abs($_POST[tsellbl]).",'".sqlzhuru($_POST[tseotit])."','".sqlzhuru($_POST[tseokey])."','".sqlzhuru($_POST[tseodes])."',".intval($_POST[Rpropx])."");$id=mysqli_insert_id($conn);
move_uploaded_file($_FILES["inp1"]['tmp_name'], "../tem/moban/".$rowcontrol[nowmb]."/homeimg/".$rowcontrol[nowmb]."Img/type1_".$id.".png");
...
$lx=intval($_GET[lx]);
$id=intval($_GET[id]);
if($lx==1){delFile($_GET[tp]);}else{delFile("../tem/moban/".$rowcontrol[nowmb]."/homeimg/".$rowcontrol[nowmb]."Img/type1_".$id.".png");delFile("../homeimg/".$rowcontrol[nowmb]."Img/type1_".$id.".png");}
...
<label><input name="Rpropx" type="radio" value="1" /> 纵向列表</label>
<label><input name="Rpropx" type="radio" value="2" /> 横向无图</label>
</li>
<li class="l1">是否推荐：</li>
<li class="l2">
...
while0("*","yjcode_type where admin=1 and id=".$_GET[id]);if(!$row=mysqli_fetch_array($res)){php_toheader("jiajctypelist.php");}
?>
<script language="javascript">
function tj(){
if((document.f1.t1.value).replace(/\s/,"")==""){alert("请输入名称！");document.f1.t1.focus();return false;}
...
<label><input name="Rpropx" type="radio"<? if($row[propx]==1){?> checked="checked"<? }?> value="1" /> 纵向列表</label>
<label><input name="Rpropx" type="radio"<? if($row[propx]==2){?> checked="checked"<? }?> value="2" /> 横向列表</label>
</li>
<li class="l1">是否推荐：</li>
<li class="l2">
...
<li class="l1">电脑端图标：</li>
<li class="l2"><input type="file" class="inp1" name="inp1" id="inp1" size="15" accept=".jpg,.gif,.jpeg,.png"><span class="fd">根据实际情况上传(每套模板都是独立的)</span></li>
<? $ntp="../tem/moban/".$rowcontrol[nowmb]."/homeimg/".$rowcontrol[nowmb]."Img/type1_".$row[id].".png";if(is_file($ntp)){?>
<li class="l8"></li>

...
$sqltask="select * from yjcode_task where id=".$id." and taskty=0";mysqli_set_charset($conn,"utf8");$restask=mysqli_query($conn,$sqltask);
if(!$rowtask=mysqli_fetch_array($restask)){php_toheader("tasklist.php");}
$bh=$rowtask[bh];

if(!empty($rowtask[useridhf])){
$sqltaskhf="select * from yjcode_taskhf where bh='".$bh."' and taskty=0 and useridhf=".$rowtask[useridhf]."";mysqli_set_charset($conn,"utf8");$restaskhf=mysqli_query($conn,$sqltaskhf);
if(!$rowtaskhf=mysqli_fetch_array($restaskhf)){php_toheader("tasklist.php");}
}
...
<div class="right">
 ...
 <? 
 while2("*","yjcode_user where id=".$rowtask[userid]);$row2=mysqli_fetch_array($res2);
 ?>
 <ul class="viewul">
 <li class="l1">任务主题：</li>

...
AdminSes_audit();
$id=$_GET[id];
while0("*","yjcode_gd where id=".$id);if(!$row=mysqli_fetch_array($res)){php_toheader("gdlist.php");}
...
if($_GET[control]=="update"){
 ...
 if(sqlzhuru($_POST[Rtz2])=="yes" && $rowcontrol[ifmob]=="on" && !empty($row[mot])){
  while1("*","yjcode_smsmb where mybh='007'");if($row1=mysqli_fetch_array($res1)){$txt=$row1[txt];}else{$txt="您的工单状态已经变更为：${zttz}";}

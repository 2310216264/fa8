...
while0("*","yjcode_jubao where bh='".$bh."'");if(!$row=mysqli_fetch_array($res)){php_toheader("jubaolist.php");}
updatetable("yjcode_jubao","zt=2 where id=".$row[id]);
if($row[admin]==1){
 while1("*","yjcode_pro where id=".$row[jbid]);if(!$row1=mysqli_fetch_array($res1)){Audit_alert("来源错误！","jubaolist.php");}
 $tit=$row1[tit];
}
...
<div class="right">
 ...
 <? while1("*","yjcode_jubaotype where admin=".$row[admin]." and zt=0 order by xh asc");while($row1=mysqli_fetch_array($res1)){?>

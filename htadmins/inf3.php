...
if(sqlzhuru($_POST[jvs])=="control"){
 ...
 $xcxpay=sqlzhuru($_POST[xcxpay0]).",".sqlzhuru($_POST[xcxpay1]);
...
$xcxpay=array("","");
if(!empty($row[xcxpay]) && $row[xcxpay]!=","){$xcxpay=preg_split("/,/",$row[xcxpay]);}
?>
...
<div class="right">
 ...
 <ul class="rcap"><li class="l1"></li><li class="l2">小程序支付</li><li class="l3"></li></ul>
 <ul class="uk">
 <li class="l1">小程序appID：</li>
 <li class="l2">
 <? if(!strstr($adminqx,",0,") && !strstr($adminqx,",0301,")){$sv="机密数据,权限不够";}else{$sv=$xcxpay[0];}?>
 <input  name="xcxpay0" value="<?=$sv?>" size="20" type="text" class="inp" /> 
 </li>
 <li class="l1">小程序密钥：</li>
 <li class="l2">
 <? if(!strstr($adminqx,",0,") && !strstr($adminqx,",0301,")){$sv="机密数据,权限不够";}else{$sv=$xcxpay[1];}?>
 <input  name="xcxpay1" value="<?=$sv?>" size="60" type="text" class="inp" /> 
 </li>
 </ul>

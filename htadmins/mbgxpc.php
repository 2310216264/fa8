...
if($_GET[control]=="update"){
 if(!strstr($adminqx,",0,")){Audit_alert("权限不够","moban.php","parent.");}
 if(panduan("*","yjcode_admin where adminuid='test'")==1){Audit_alert("请先删除test管理员账户","moban.php","parent.");}
 zwzr();
 $mb=sqlzhuru($_POST[t1]);
 if(empty($mb) || !preg_match("/^[_a-zA-Z0-9]*$/",$mb)){Audit_alert("模板名称无效","moban.php","parent.");}
 createDir("../tem/moban/".$mb."/");
 $up1=$_FILES["inp1"]["name"];
 $mc=$mb.".".returnhz($up1);
 move_uploaded_file($_FILES["inp1"]['tmp_name'],"../tem/moban/".$mb."/".$mc);
 $zip_filename = "../tem/moban/".$mb."/".$mc;
 ...
 if($rs !== TRUE)
 {die('Error Code:1001');}
 $zip->extractTo("../tem/moban/".$mb."/");
 $zip->close();
 Audit_alert("模板更新成功，点击启用可以看效果啦","moban.php","parent.");
...
function tj(){
 ...
 f1.action="mbgxpc.php?control=update";
}
</script>
</head>
<body style="overflow-x:hidden;">
 ...
 <li class="l4">请务必只上传官方原版<strong>【电脑端】</strong>模板，上传其他文件导致的网站崩溃问题均不免费受理</li>
 </ul>

...
if($_GET[control]=="add"){
 ...
 intotable("yjcode_type","admin,type1,type2,xh,sj,buyform,seotit,seokey,seodes","2,'".sqlzhuru($_POST[t0])."','".sqlzhuru($_POST[t1])."',".sqlzhuru($_POST[t2]).",'".$sj."','".sqlzhuru($_POST[buyform])."','".sqlzhuru($_POST[tseotit])."','".sqlzhuru($_POST[tseokey])."','".sqlzhuru($_POST[tseodes])."'");$id=mysqli_insert_id($conn);
...
while0("*","yjcode_type where id=".$ty1id);if(!$row=mysqli_fetch_array($res)){php_toheader("typelist.php");}
?>
...
<div class="right">
 ...
 <? if($_GET[t]=="suc"){systs("恭喜您，操作成功！【<a href='type2.php?ty1id=".$ty1id."'>继续添加</a>】","type2.php?action=".$_GET[action]."&ty1id=".$ty1id."&id=".$_GET[id]);}?>
 ...
 <li class="l2"><input type="text" class="inp" size="70" name="tseotit" /></li>
 ...
 while0("*","yjcode_type where admin=2 and id=".$_GET[id]);if(!$row=mysqli_fetch_array($res)){php_toheader("typelist.php");}
 ?>
 <script language="javascript">
 function tj(){
 if((document.f1.t1.value).replace(/\s/,"")==""){alert("请输入名称！");document.f1.t1.focus();return false;}

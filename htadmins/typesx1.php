...
while0("*","yjcode_typesx where admin=1 and id=".$ty1id);$row=mysqli_fetch_array($res);$typeid=$row[typeid];
$sj=date("Y-m-d H:i:s");
//函数开始
if($_GET[control]=="add"){
 ...
 move_uploaded_file($_FILES["inp1"]['tmp_name'], "../tem/moban/".$rowcontrol[nowmb]."/homeimg/".$rowcontrol[nowmb]."Img/typesx2_".$id.".png");
...
<div class="right">
 ...
 <? if($_GET[t]=="suc"){systs("恭喜您，操作成功！【<a href='typesx1.php?ty1id=".$ty1id."'>继续添加</a>】","typesx1.php?action=".$_GET[action]."&ty1id=".$_GET[ty1id]."&id=".$_GET[id]);}?>
 <div class="bqu1">
 ...
 $ty1id=intval($_GET[ty1id]);
 $id=intval($_GET[id]);
 while1("*","yjcode_typesx where id=".$id);if(!$row1=mysqli_fetch_array($res1)){php_toheader("typesxlist.php?typeid=".$typeid);}
 ?>
 <script language="javascript">
 function tj(){
 if((document.f1.t1.value).replace(/\s/,"")==""){alert("请输入选项名称！");document.f1.t1.focus();return false;}
 ...
 <li class="l1">图标：</li>
 <li class="l2"><input type="file" class="inp1" name="inp1" id="inp1" size="15" accept=".jpg,.gif,.jpeg,.png"><span class="fd">根据实际情况上传(每套模板都是独立的)</span></li>
 <? $ntp="../tem/moban/".$rowcontrol[nowmb]."/homeimg/".$rowcontrol[nowmb]."Img/typesx2_".$row1[id].".png";if(is_file($ntp)){?>

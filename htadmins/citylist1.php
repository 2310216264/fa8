...
AdminSes_audit();
$pid=$_GET[pid];
while1("*","yjcode_city where bh='".$pid."'");if(!$row1=mysqli_fetch_array($res1)){php_toheader("citylist.php");}
$city1=$row1[name1];
?>
...
<div class="right">
 ...
 <?
 while1("*","yjcode_city where level=2 and parentid='".$pid."' order by xh asc");while($row1=mysqli_fetch_array($res1)){
 ...
 <?
 while2("*","yjcode_city where level=3 and parentid='".$row1[bh]."' order by xh asc");while($row2=mysqli_fetch_array($res2)){

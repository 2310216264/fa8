...
if($_POST[jvs]=="qd"){
 ...
 $lxd=0;
 while1("*","yjcode_qiandaojf order by daynum desc limit 1");if($row1=mysqli_fetch_array($res1)){
  for($i=2;$i<=$row1[daynum];$i++){
   $iv=$i-1;
   $sjv=date("Y-m-d",strtotime("-".$iv." day"));
   $sj1=$sjv." 00:00:00";
   $sj2=$sjv." 23:59:59";
   while2("*","yjcode_qiandao where userid=".$userid." and sj>='".$sj1."' and sj<='".$sj2."'");if(!$row2=mysqli_fetch_array($res2)){break;}else{$lxd++;}
  }
  if($lxd>0){
  $dnum=$lxd+1;
  while3("*","yjcode_qiandaojf where daynum<=".$dnum." order by daynum desc limit 1");if($row3=mysqli_fetch_array($res3)){$qdjf=$row3[jf];$lx="(连续签到".$dnum."天)";}
  }
 }
...
<div class="userright">
 ...
 pagef($ses,30,"yjcode_qiandao","order by sj desc");while($row=mysqli_fetch_array($res)){
 ?>
 <ul class="qiandao">

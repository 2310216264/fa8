...
if($_POST[jvs]=="openshop"){
 ...
 while1("*","yjcode_openyue where id=".$d[2]);if(!$row1=mysqli_fetch_array($res1)){Audit_alert("操作失败，返回重试！","openshop4.php");}
 if(intval($d[3])==1 && $row1[money1]==0){Audit_alert("操作失败，返回重试！","openshop4.php");}
 $m=$row1[money1];
...
<div class="userright">
 ...
 <? 
 $i=1;
 while1("*","yjcode_openyue order by yue asc");while($row1=mysqli_fetch_array($res1)){if($row1[yue] % 12==0){$nd=$row1[yue]/12;$nd=$nd."年";}else{$nd=$row1[yue]."个月";}
 if($i>1 || $row1[money1]>0){
 ?>
 <option value="<?=$row1[yue]?>xcf<?=$row1[money1]?>xcf<?=$row1[id]?>xcf<?=$i?>"><?=$nd?> (费用：<?=$row1[money1]?>元)</option>
 <? }$i++;}?>
 </select>
 </li>

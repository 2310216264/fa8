...
<div class="userright">
 ...
 <div class="ksedi">
 <div class="d1">
 <a href="propjlist.php?ifhf=no" class="a1">查看所有未回复评价(共<?=returncount("yjcode_order where selluserid=".$luserid." and admin=2 and ifpj=1 and (hftxt='' or hftxt is null)")?>个)</a>
 </div>
 <div class="d2">
 ...
 <?
 $ses=" where admin=2 and ifpj=1 and selluserid=".$luserid;
 if($_GET[ifhf]=="no"){$ses=$ses." and (hftxt='' or hftxt is null)";}
 if($_GET[ifhf]=="yes"){$ses=$ses." and hftxt<>''";}
 if($_GET[st1]!=""){$ses=$ses." and pjtxt like '%".$_GET[st1]."%'";}
 $page=$_GET["page"];if($page==""){$page=1;}else{$page=intval($_GET["page"]);}
 pagef($ses,10,"yjcode_order","order by sj desc");while($row=mysqli_fetch_array($res)){
 while1("*","yjcode_pro where bh='".$row[probh]."'");$row1=mysqli_fetch_array($res1);
 ?>
 <ul class="propjlist">
 <li class="l1">商品信息：</li>
 ...
 <ul class="propjlist1">
 <li class="l1">回复内容：</li>
 <li class="l2" style="cursor:pointer;" onclick="hfonc(<?=$row[id]?>)"><? if(empty($row[hftxt])){?><span class="hui">【暂未回复，点击进行回复】</span><? }else{?><span class="green">回复时间：<?=$row[hfsj]?><br>回复内容：<?=$row[hftxt]?></span><? }?></li>
 </ul>
 <? }?>

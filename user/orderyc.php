...
if(sqlzhuru($_POST[jvs])=="yc" && empty($row[ycshsj]) && $row[ddzt]=="db"){
 zwzr();
 $oksj=date("Y-m-d H:i:s",strtotime ("+".$rowcontrol[ycsj]." day",$row[dbautosj]));
 updatetable("yjcode_order","ycshsj='".$sj."',dbautosj=".strtotime($oksj)." where zuorderbh='".$zuorderbh."'");
...
<div class="userright">
 ...
 f1.action="orderyc.php?zuorderbh=<?=$zuorderbh?>";
 }
 </script>

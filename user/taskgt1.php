...
$sqltaskhf="select * from yjcode_taskhf where bh='".$bh."' and useridhf=".$userid." and taskty=1 and id=".$hfid."";mysqli_set_charset($conn,"utf8");$restaskhf=mysqli_query($conn,$sqltaskhf);
if(!$rowtaskhf=mysqli_fetch_array($restaskhf)){php_toheader("tasklist1.php");}

$sqltask="select * from yjcode_task where bh='".$bh."' and taskty=1";mysqli_set_charset($conn,"utf8");$restask=mysqli_query($conn,$sqltask);
...
<div class="userright">
 ...
 <div class="taskgtlist">
  ...
  <? 
  while1("*","yjcode_tasklog where bh='".$bh."' and userid=".$rowtask[userid]." and useridhf=".$userid." order by sj asc");while($row1=mysqli_fetch_array($res1)){
  $txt=$row1[txt];

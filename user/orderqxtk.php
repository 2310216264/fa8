...
if(sqlzhuru($_POST[jvs])=="qxtk" && $row[ddzt]=="back"){
 ...
 if(panduan("uid,zfmm","yjcode_user where zfmm='".$zfmm."' and uid='".$_SESSION[SHOPUSER]."'")==0){Audit_alert("支付密码有误！","orderqxtk.php?zuorderbh=".$zuorderbh);}
 if($row[ddzt]!="back"){Audit_alert("未知错误！","orderqxtk.php?zuorderbh=".$zuorderbh);}
 $oksj=date("Y-m-d H:i:s",strtotime("-30 second"));
 if(empty($row[fhsj])){
  updatetable("yjcode_order","ddzt='wait' where zuorderbh='".$zuorderbh."'");
 }else{
  updatetable("yjcode_order","ddzt='db',dbautosj=".strtotime($oksj)." where zuorderbh='".$zuorderbh."'");
 }
 $str="买家取消了退款申请";
 intotable("yjcode_orderlog","orderbh,userid,selluserid,admin,txt,sj","'".$orderbh."',".$row[userid].",".$row[selluserid].",1,'".$str."','".getsj()."'");
...
<div class="userright">
 ...
 f1.action="orderqxtk.php?zuorderbh=<?=$zuorderbh?>";
 }
 </script>
 <form name="f1" method="post" onsubmit="return tj()">
 <ul class="ordercz">
 <li class="l1"><strong>取消退款申请的话，已发货的订单会<span class="red">自动转为交易成功</span>，请务必与卖家沟通好，确认商品没有问题的前提下，再进行该操作</strong></li>

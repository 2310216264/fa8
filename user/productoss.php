...
$userid=$rowuser[id];
$bh=$_GET[bh];
while0("*","yjcode_pro where bh='".$bh."' and userid=".$userid);if(!$row=mysqli_fetch_array($res)){php_toheader("productlist.php");}

while0("control_keywords","yjcode_control");$rowss=mysqli_fetch_array($res);

if($rowss[control_keywords]){
	$a = $rowss[control_keywords];
}else{
	$a = '源码交易网,源码商城,交易平台,网站交易,源码交易,站长交易,程序交易,源码分享,商业源码,任务交易,免费源码,A8站';
}
$d = explode(',',$a);
$e = rand(0,count($d));
$f = $d[$e];
...
if($_GET[control]=="update"){
 ...
 if($money1<0 || $money2<0 || $money3<0){Audit_alert("价格不能为负数！","productosslist.php");}
 $fhxs=intval(sqlzhuru($_POST[Rfhxs]));
 if($rowcontrol[ifproductoss]=="on"){$nzt=0;}else{$nzt=1;}
 ...
 @updatetable("yjcode_pro",$ses."
			 ...
			 myty1id=".intval($myty[0]).",
			 myty2id=".intval($myty[1]).",
			 tysx='".$tysx."',
			 zt=".intval($nzt).",
			 tit='".$tit."',
...
tjwait();
f1.action="productoss.php?bh=<?=$bh?>&control=update";
}
...
function taocanonc(){
	layer.open({
		...
		skin: 'layui-layer-rim', //加上边框
		content:['taocanlist.php?bh=<?=$bh?>', 'yes'] 
	});
}

function taocankconc(){
	layer.open({
		type:2,
		shadeClose: false,
		scrollbar: true,
		area: ['960px', '550px'],
		title:["库存管理","text-align:left"],
		//skin: 'layui-layer-rim', //加上边框
		content:['kclist.php?bh=<?=$bh?>', 'yes'] 
	});
}
...
<div class="userright">
 ...
 [<a href="productosslx.php?action=update&id=<?=$row[id]?>">修改</a>]
 </li>
 <li class="l1">自定义分组：</li>
 <li class="l2">
 <select name="mty" class="inp">
 <option value="0yjcode0">选择分组</option>
 <? while1("*","yjcode_protype where admin=1 and zt=0 and userid=".$luserid);while($row1=mysqli_fetch_array($res1)){?>
 ...
 <? while2("*","yjcode_protype where admin=2 and name1='".$row1[name1]."' and zt=0 and userid=".$luserid);while($row2=mysqli_fetch_array($res2)){?>
 ...
 <? for($i=1;$i<10;$i++){?>
 <option value="1.<?=$i?>">X1.<?=$i?>倍</option>
 <? }?>
 </select>
 </li>
 </ul>
 ...
 <li class="l1">库存：</li>
 <li class="l21"><strong class="red"><?=$row[kcnum]?>件</strong> [<a href="javascript:void(0);" onclick="taocankconc()" class="blue">管理库存</a>]</li>
 <li class="l1">说明：</li>
 ...
 <? $i=1;while1("*","yjcode_typesx where admin=1 and typeid=".$row[ty1id]." order by xh asc");while($row1=mysqli_fetch_array($res1)){?>
 <input type="hidden" value="<?=$row1[id]?>" name="tysxty1_<?=$i?>" />
 <ul class="uk1">
 <li class="l1"><?=$row1[name1]?>：</li>
 <li class="l2">
 <? $j=1;while2("*","yjcode_typesx where admin=2 and name1='".$row1[name1]."' and typeid=".$row1[typeid]." order by xh asc");while($row2=mysqli_fetch_array($res2)){?>
 ...
 <? 
 $j=1;while1("*","yjcode_userdj where zt=0 order by xh asc");while($row1=mysqli_fetch_array($res1)){
 while2("*","yjcode_prouserdj where probh='".$bh."' and djname='".$row1[name1]."'");if($row2=mysqli_fetch_array($res2)){$zhekou=$row2[zhi];}else{$zhekou=$row1[zhekou];}
 ?>
 <ul class="dju2">
 ...
 <? while1("*","yjcode_txtmb where admin=1 order by mbid asc");while($row1=mysqli_fetch_array($res1)){?>
 ...
 <ul class="uk uk0">
 <li class="l3"><? tjbtnr("提交","productosslist.php")?></li>
 </ul>
 </form>

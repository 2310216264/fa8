...
$sj=date("Y-m-d H:i:s");
if($_GET[control]=="del"){
deletetable("yjcode_shopfav where userid=".$userid." and id=".intval($_GET[id]));
...
<div class="userright">
 ...
 pagef(" where userid=".$userid,10,"yjcode_shopfav","order by sj desc");while($row=mysqli_fetch_array($res)){
 while1("*","yjcode_user where id=".$row[shopid]);$row1=mysqli_fetch_array($res1);
 ?>
 <div class="favshop">
 <ul class="u1">
 ...
 <ul class="u2">
 <? while1("*","yjcode_pro where userid=".$row[shopid]." and ifxj=0 and zt=0 order by lastsj desc limit 5");while($row1=mysqli_fetch_array($res1)){?>
 <li class="l1">
 <a href="../product/view<?=$row1[id]?>.html" target="_blank" title="<?=$row1[tit]?>">
 <img border="0" src="<?=returntp("bh='".$row1[bh]."' order by iffm desc limit 1","-2")?>" width="100" height="100" />
 <?=$row1[tit]?>
 </a>

...
$userid=$rowuser[id];
while0("*","yjcode_shopbannar where userid=".$userid." and bh='".$bh."'");if(!$row=mysqli_fetch_array($res)){Audit_alert("来源错误","shopbannarlist.php","parent.");}
$at="../upload/".$row[userid]."/shopbannar_".$row[bh].".jpg";

if($_GET[control]=="update"){
 zwzr();
 updatetable("yjcode_shopbannar","tit='".sqlzhuru($_POST[ttit])."',aurl='".sqlzhuru($_POST[taurl])."',xh=".intval($_POST[txh]).",targ=".sqlzhuru($_POST[R1]).",zt=0 where userid=".$userid." and id=".$row[id]);

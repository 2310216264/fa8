...
while0("*","yjcode_pro where userid=".$userid." and bh='".$bh."'");if(!$row=mysqli_fetch_array($res)){php_toheader("productlist.php");}
$tit=$row[tit];

$ty1id=intval($_GET[ty1id]);
while0("*","yjcode_taocan where userid=".$userid." and id=".$ty1id);$row=mysqli_fetch_array($res);
$ty1tit=$row[tit];
...
<div class="userright">
 ...
 <? systs("恭喜您，操作成功!","taocan.php?id=".$id."&bh=".$bh."&ty1id=".$ty1id)?>
 ...
 <label><input name="Rfhxs" type="radio" value="3" onclick="fhxsonc(3)" <? if(3==$row[fhxs]){?>checked="checked"<? }?> /> 网站下载(独立)</label>
 <? }?>

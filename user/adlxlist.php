...
if($_GET[control]=="sd"){
 ...
 while1("*","yjcode_ad where zt=1 and userid=".$userid." and id=".intval($_GET[id]));if(!$row1=mysqli_fetch_array($res1)){php_toheader("adlxlist.php?zt=1");}
 ...
 deletetable("yjcode_ad where id=".intval($_GET[id]));
...
<div class="userright">
 ...
 pagef($ses,30,"yjcode_ad","order by sj desc");while($row=mysqli_fetch_array($res)){
 autoAD($row[adbh]);
 ?>

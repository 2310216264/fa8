...
if(sqlzhuru($_POST[yjcode])=="close"){
 ...
 if(panduan("uid,zfmm","yjcode_user where zfmm='".$zfmm."' and uid='".$_SESSION[SHOPUSER]."'")==0){Audit_alert("支付密码有误！","serverclose.php?orderbh=".$orderbh);}
 if($row[ddzt]!=1 && $row[ddzt]!=2){Audit_alert("未知错误！","serverorderview.php?orderbh=".$orderbh);}
 updatetable("yjcode_serverorder","ddzt=13 where userid=".$userid." and id=".$row[id]);
...
<div class="userright">
 ...
 f1.action="serverclose.php?orderbh=<?=$orderbh?>";
 }
 </script>
 ...
 * <span class="red">由于订单未进行支付，取消订单，双方无责。不过依然建议与商家沟通协商好</span><br>
 </li>

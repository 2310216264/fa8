...
if($admin==1){ //商品B
 ...
 while1("*","yjcode_order where zuorderbh='".$bh."' and userid=".$userid);if(!$row1=mysqli_fetch_array($res1)){Audit_alert("登录超时","../reg/","parent.");}
 $targetFolder = "upload/".$userid."/".$bh."/";
 createDir("../".$targetFolder);
 $total = count($_FILES['inp1']['tmp_name']);
 for($k=0; $k<$total; $k++) {
 ...
 while1("*","yjcode_news where bh='".$bh."' and userid=".$userid);if(!$row1=mysqli_fetch_array($res1)){Audit_alert("登录超时","../reg/","parent.");}
 ...
 for($k=0; $k<$total; $k++) {
  if(returncount("yjcode_tp where bh='".$bh."'")<7){
   if(is_uploaded_file($_FILES['inp1']['tmp_name'][$k])){
	...
	$nxh=returnxh("yjcode_tp"," and bh='".$bh."'");
	//OSSB
	if(check_in("a3a",$rowcontrol[aliosskg])){
		$upty=1;
		include('../config/alioss/Common.php');
		$bucket = Common::getBucketName();
		$ossClient = Common::getOssClient();
 ...
 for($k=0; $k<$total; $k++) {
  if(returncount("yjcode_tp where bh='".$bh."'")<7){
   if(is_uploaded_file($_FILES['inp1']['tmp_name'][$k])){
	...
	$wjv=$targetFolder.$mbh.".jpg";
	$nxh=returnxh("yjcode_tp"," and bh='".$bh."'");
	intotable("yjcode_tp","bh,tp,type1,sj,userid,xh,upty","'".$bh."','".$wjv."','服务','".$sj."',".$userid.",".$nxh.",".intval($upty)."");
   }
  }
 }


}elseif($admin==8){ //服务评价B
 while1("*","yjcode_serverorder where orderbh='".$bh."' and userid=".$userid);if(!$row1=mysqli_fetch_array($res1)){Audit_alert("登录超时","../reg/","parent.");}
 $targetFolder = "upload/".$userid."/".$bh."/";
 createDir("../".$targetFolder);
 $total = count($_FILES['inp1']['tmp_name']);
 for($k=0; $k<$total; $k++) {
  if(returncount("yjcode_tp where bh='".$bh."'")<7){
   if(is_uploaded_file($_FILES['inp1']['tmp_name'][$k])){
	...
	$wjv=$targetFolder.$mbh.".jpg";
	$nxh=returnxh("yjcode_tp"," and bh='".$bh."'");
	intotable("yjcode_tp","bh,tp,type1,sj,userid,xh","'".$bh."','".$wjv."','服务评价','".$sj."',".$userid.",".$nxh."");
   }
  }
 }
